...
export default function MapPage() {
  ...
  useEffect(() => {
    if (!startPoint || mapTasks.length === 0) { setOrderedIds([]); setIsRouteReady(false); return; }
    const ordered = orderTasksWithTime(startPoint, mapTasks);
    setOrderedIds(ordered.map(t => t.id));
    setIsRouteReady(false);
  }, [mapTasks, startPoint]);

  const handleRouteReady = useCallback((ready: boolean) => { setIsRouteReady(ready); }, []);

  useEffect(() => { setIsRouteReady(false); setOrderedIds([]); }, [startPoint]);
  ...
  const updateStatus = async (id: string, status: 'pending' | 'inprogress' | 'complete') => {
    ...
    setMapTasks(prev => {
      ...
      const today = todayStr();
      const todayWorks = works.map(w => w.id === id ? { ...w, status } : w).filter(w => w.work_date === today);
      const allDone = todayWorks.length > 0 && todayWorks.every(w => w.status === 'complete' || (w.id === id && status === 'complete'));
      if (allDone) { setAllDoneToday(true); setShowReturnRoute(true); }
      return updated;
    });
    setShowModal(false);
  };
  ...
  const handleConfirmReturn = () => {
    setShowReturnRoute(false); setAllDoneToday(false);
    setMapTasks([]); setOrderedIds([]); setIsRouteReady(false);
  };
  ...
  return (
    ...
    <div className="min-h-screen bg-slate-50 lg:pl-20 pb-24 lg:pb-6">
      <div className="max-w-4xl mx-auto p-4 space-y-4">
        ...
        <header className="bg-white p-4 md:p-6 rounded-[2rem] shadow-sm border border-slate-100">
          ...
          <div className="flex items-center justify-between gap-3 mb-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-slate-900 rounded-2xl text-white shadow-xl shrink-0">
                <Navigation size={20} />
              </div>
              <div>
                        ...
                        <button key={emp.id} onClick={() => { setSelectedEmployeeId(emp.id); setEmpDropdownOpen(false); setEmpSearch(''); }}
                          ...
                          <div className="text-left min-w-0">
                            ...
                            <p className="text-[10px] opacity-60 uppercase font-black truncate">
                              {emp.departments?.name}{emp.staff_id && ` · #${emp.staff_id}`}
                            </p>
                          </div>
                        </button>
                      ))}
          ...
          <div className="border-t border-slate-100 pt-4">
            ...
            {/* ✅ บน mobile stack เป็น column, บน sm ขึ้นไปเป็น row */}
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              {/* ✅ GPS button เต็มความกว้างบน mobile */}
              <button
                type="button"
        ...
        {/* ── All done banner ── */}
        {allDoneToday && (
          <div className="bg-emerald-50 border-2 border-emerald-200 rounded-[2rem] p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <div className="p-2.5 bg-emerald-500 rounded-2xl text-white shrink-0"><CheckCircle2 size={20} /></div>
              <div>
                <p className="font-black text-emerald-800">งานวันนี้ครบแล้ว!</p>
                <p className="text-xs text-emerald-600 font-bold">เส้นสีส้มแสดงเส้นทางกลับ กดยืนยันเพื่อจบงานวันนี้</p>
              </div>
            </div>
            <button onClick={handleConfirmReturn}
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-3 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-black text-sm shadow-lg active:scale-95 transition-all">
              <Home size={16} /> จบงานวันนี้แล้ว
            </button>
          </div>
        )}
        ...
        {!selectedEmployeeId ? (
          ...
          <>
            {/* ── Map ── */}
            {/* ✅ height บน mobile เล็กกว่า ไม่กินพื้นที่มากเกิน */}
            <div className="bg-white p-2 rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden"
              ...
              {mapTasks.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-slate-300">
                  ...
                  <p className="font-bold text-sm">ไม่มีงานที่มีพิกัด</p>
                  <p className="text-xs mt-1 text-slate-400 text-center px-4">งานจะแสดงเมื่อมีการบันทึก lat/lng จากชื่อสถานที่</p>
                </div>
              )}
            </div>
                ...
                <div className="space-y-3 relative">
                  ...
                  {orderedTasks.map((task, idx) => {
                    ...
                    return (
                      <div key={task.id} className={`flex gap-3 items-center transition-opacity ${isComplete ? 'opacity-40' : ''}`}>
                        ...
                        <div className="flex-grow p-3 bg-slate-50 rounded-2xl flex justify-between items-center min-w-0">
                  ...
                  {allDoneToday && (
                    <div className="flex gap-3 items-center">
                      <div className="z-10 w-8 h-8 rounded-full bg-orange-500 text-white flex items-center justify-center text-xs font-black shadow-lg ring-4 ring-white shrink-0">🏠</div>
                      <div className="flex-grow p-3 bg-orange-50 rounded-2xl border border-orange-100">
                        ...
                        <p className="text-xs text-orange-400 font-bold mt-0.5 truncate">{startInput}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="space-y-3">
              <h2 className="font-black text-slate-800 px-1 text-sm">งานที่รอดำเนินการ ({works.length})</h2>
              {works.length === 0 ? (
                <div className="bg-white rounded-[2rem] p-12 text-center border border-slate-100 text-slate-300">
                  <p className="font-bold text-sm">{allDoneToday ? 'งานวันนี้เสร็จสิ้นทั้งหมด' : 'ไม่มีงานค้าง'}</p>
                </div>
              ) : (
                works.map(work => {
      ...
      {showModal && selectedWork && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[200] flex items-end sm:items-center justify-center p-0 sm:p-4"
            ...
            {/* ✅ บน mobile modal ขึ้นมาจากด้านล่าง (bottom sheet) */}
            <div className="bg-white w-full sm:max-w-lg sm:rounded-[2.5rem] rounded-t-[2.5rem] shadow-2xl relative overflow-hidden"
              onClick={e => e.stopPropagation()}>
              <div className="h-3 w-full" style={barStyle} />
              {/* ✅ drag handle บน mobile */}
              ...
              <div className="p-6 sm:p-10">
                <div className="flex justify-between items-start mb-6">
                  ...
                  <button onClick={() => setShowModal(false)} className="p-2.5 bg-slate-50 rounded-2xl text-slate-400 hover:bg-slate-100 shrink-0 ml-3">
                    <X size={18} />
                  </button>
                </div>
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3 text-sm font-bold">
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      ...
                      <div className="flex items-center gap-2 text-slate-700 text-sm"><Building2 size={14} className="text-blue-500 shrink-0" /><span className="truncate">{selectedWork.department}</span></div>
                    </div>
                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                      <p className="text-[10px] text-slate-400 uppercase mb-1.5 font-black tracking-wider">เวลานัดหมาย</p>
                      <div className="flex items-center gap-2 text-slate-700 text-sm"><Clock size={14} className="text-blue-500" />{selectedWork.work_time} น.</div>
                    </div>
                  </div>
                  <div className="p-6 rounded-[2rem] bg-slate-900 text-white font-bold shadow-xl">
                    ...
                    <p className="text-base leading-relaxed">{selectedWork.detail}</p>
                  </div>
                </div>
