...
function ResetPasswordModal({ 
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-sm bg-white rounded-[2rem] border-4 border-white shadow-2xl p-8 space-y-5">
                {/* Header */}
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h2 className="text-lg font-black text-slate-900">รีเซ็ตรหัสผ่าน</h2>
                        <p className="text-xs font-bold text-slate-400">
                            {employee.name}
                            <span className="ml-1 text-slate-300">#{employee.staff_id}</span>
                        </p>
                    </div>
                </div>
                ...
                {success ? (
                    ...
                    <>
                        <div className="space-y-3">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-11 p-4 bg-slate-50 border-2 border-transparent focus:border-slate-900 rounded-2xl font-bold outline-none text-sm"
                                />
                            </div>
                            <div className="relative">
                                <Lock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={16} />
                        ...
                        {error && (
                            <p className="text-xs font-bold text-rose-500 bg-rose-50 border-2 border-rose-100 px-4 py-3 rounded-2xl">
                                {error}
                            </p>
                        )}
                        ...
                        <div className="flex gap-2 pt-1">
                            <button
                                onClick={onClose}
                                className="flex-1 py-3.5 bg-slate-100 text-slate-500 rounded-[1.5rem] font-black hover:bg-slate-200 transition-all text-sm"
                            >
                                ยกเลิก
                            </button>
                            <button
                                onClick={handleReset}
                                disabled={loading}
                                className="flex-1 py-3.5 bg-slate-900 text-white rounded-[1.5rem] font-black hover:bg-slate-800 disabled:opacity-50 transition-all text-sm flex items-center justify-center gap-2"
                            >
...
export default function EmployeesPage() {
    ...
    const handleDisable = async (emp: Employee) => {
        if (!confirm(`ย้ายคุณ ${emp.name} ไปที่หน้าพนักงานที่ออกแล้ว?\n(บัญชีผู้ใช้จะถูกลบทันทีเพื่อความปลอดภัย)`)) return;

        try {
            // 1. ลบบัญชีใน auth.users ทันที เพื่อไม่ให้ล็อกอินได้ระหว่างอยู่ใน disabled
            if (emp.user_id) {
                const res = await fetch('/employees/api', {
                    method: 'DELETE',
                    headers: { 'Content-Type': 'application/json' },
            ...
            // 2. set is_active = false และล้าง user_id
                            ...
                            {filteredEmployees.map((emp) => (
                                <div key={emp.id} className={`group p-4 rounded-[2rem] border-2 bg-white flex items-center gap-4 transition-all ${editId === emp.id ? 'border-orange-200 bg-orange-50/30 shadow-inner' : 'border-slate-50 hover:shadow-xl'}`}>
                                    ...
                                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button 
                                            onClick={() => { setIsEditing(true); setEditId(emp.id); setFormData({ name: emp.name, staff_id: emp.staff_id, department_id: emp.department_id || "", email: "", password: "" }); setPreviewUrl(emp.image_url); window.scrollTo({ top: 0, behavior: 'smooth' }); }} 
                                            className="p-2 text-slate-300 hover:text-orange-500"
                                            title="แก้ไขข้อมูล"
                                        >
                                            <Edit3 size={16} />
                                        </button>
                                        {/* ปุ่ม KeyRound แสดงเฉพาะพนักงานที่มีบัญชี */}
                                        {emp.user_id && (
                                            <button 
                                                onClick={() => setResetTarget(emp)} 
                                                className="p-2 text-slate-300 hover:text-sky-500"
