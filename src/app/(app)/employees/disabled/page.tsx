...
export default function DisabledEmployeesPage() {
    ...
    return (
        <main className="max-w-5xl mx-auto p-3 md:p-8 min-h-screen bg-slate-50/30">
            {/* ── Header ── */}
            <header className="mb-5 md:mb-8 flex flex-col md:flex-row md:items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                    <div className="p-2.5 md:p-3 bg-slate-900 rounded-xl md:rounded-2xl text-white shadow-xl"><UserX size={20} /></div>
                    <div>
                ...
                <button onClick={() => router.push("/employees")} className="text-slate-400 hover:text-slate-900 font-bold text-sm flex items-center gap-1.5 px-2 transition-colors">
            ...
            <div className="bg-white rounded-[1.5rem] md:rounded-[2.5rem] border-4 border-white shadow-xl overflow-hidden min-h-[300px]">
                ...
                <div className="p-4 md:p-6 space-y-3">
                    {loading ? (
                        <div className="py-16 flex justify-center"><Loader2 className="animate-spin text-slate-200" size={36} /></div>
                    ) : filtered.length === 0 ? (
                        ...
                        filtered.map((emp) => (
                            <div key={emp.id} className="p-3.5 md:p-4 rounded-[1.5rem] border-2 border-slate-50 bg-white flex items-center gap-3 group transition-all hover:border-slate-100 shadow-sm hover:shadow-md">
                                ...
                                <div className="flex gap-2 shrink-0">
                                    ...
                                    >
                                        <UserCheck size={14} /> 
                                        <span className="hidden sm:inline">ดึงกลับ</span>
                                        <span className="sm:hidden">ดึงกลับ</span>
                                    </button>
                                    <button 
